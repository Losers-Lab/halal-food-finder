...
| **Image storage** | **MinIO self-hosted** (S3-compatible, **$0**, runs in Docker alongside the app) — private bucket, presigned PUT/GET, namespaced keys, versioning + content SHA-256 audit | Founder decision (cost-conscious): fully $0, self-operated. Switching to Cloudflare R2 / AWS S3 later is config (S3 SDK on configurable endpoint), not code. |
